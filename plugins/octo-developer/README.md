...
`octo-developer` 为 `Mininglamp-OSS/octo-server` GitHub PR 提供 review 观察、修复闭环和当前 head 安全合并流程。它包含：

- `octo_pr` Extension 工具：每个会话一个后台 watcher，缓存 review/评论变化并在变化时唤醒会话；
- `octo-pr` Skill：收集正式 review、行内评论和普通 PR 评论，判断当前 head 的批准门禁，完成修复—测试—回复—push—新一轮，并执行受保护的 merge gate。
...
Extension 在会话创建时加载，安装后请重启 `omp` 或开启新会话；Skill 可通过 `/reload-plugins` 刷新。插件不安装 `gh` 或其他运行时依赖，需自行准备已登录且有权限的 GitHub CLI。
...
| `fresh` | `boolean` | 仅 `status` 可用；`true` 立即读取 GitHub，通常只在合并前使用 |
...
当前 head 上两位**不同** reviewer 的最新 decisive `APPROVED` 且 review commit 绑定同一当前 head 才满足票数；旧 head 批准不计数。任一尚未被后续 decisive `APPROVED` 或 `DISMISSED` 解除的 `CHANGES_REQUESTED`（包括旧 head 上的）都阻塞；`COMMENTED` 不会清除它。两票满足后不等待第三位 reviewer。push 产生新 head 后必须重新取得批准，但不能丢弃仍有效的旧拒绝。`ready` 只表示 review 票数满足，不代表实时 GitHub merge gate 已通过；合并前仍必须 fresh 核对 PR facts、required checks 和 blocking threads。
...
用户授权合并且门禁满足时，Skill 在合并前调用 `status` 的 `fresh: true`，重新确认同一个 `headSha`，并确认 PR 为 `OPEN`、`mergeable` 为 `MERGEABLE`、`mergeStateStatus` 为 `CLEAN`、required checks 全部通过且没有未解决的 blocking threads，再执行：

```sh
gh pr merge "$PR" --squash --match-head-commit "$HEAD_SHA"
```

命令返回后必须再次 `status`（`fresh: true`）确认 PR `state=MERGED` 或 `CLOSED`；确认终态后立即调用 `{"action":"cancel"}` 结束 watcher，才能报告任务已收尾。`gh pr merge` 可能只进入 merge queue/auto-merge；queued、pending 或其他非终态结果只能如实报告并继续按任务需要观察。若两票已满足但用户未授权 merge，且任务只是完成 review 等待，则汇报 `ready` 后同样调用 `cancel`，不要让后台 watcher 无限查询。禁止 `--admin`、`--auto`、`--delete-branch` 以及移除 `--match-head-commit` 的弱化重试；watcher 本身永远不执行合并或远端写入。
...
- 另以真实 GitHub PR887 做只读 `fetchSnapshot` 验证。以上运行未包含真实一小时等待、真实 GitHub merge、发布或安装验收；`ready` 仍只表示 review 票数，不能替代合并前的 fresh gate。
